...
public class BatchPdfGenerator {
    ...
    public static boolean generatePdfBatch(List<String> rawHtmlList, String outputPath, boolean fitToPage) {
        ...
        if (rawHtmlList.size() <= BATCH_SIZE) {
            return processChunk(rawHtmlList, new File(outputPath), fitToPage);
        }
        ...
        try {
            for (int i = 0; i < total; i += BATCH_SIZE) {
                ...
                boolean success = processChunk(subList, tempPdf, fitToPage);
                if (!success) {
     ...
     public static boolean generatePdf(List<String> rawHtmlList, String outputPath, boolean fitToPage) {
        if (rawHtmlList == null || rawHtmlList.isEmpty()) {
            return false;
        }
        try (OutputStream os = new BufferedOutputStream(new FileOutputStream(outputPath))) {
            
            StringBuilder batchHtml = new StringBuilder(rawHtmlList.size() * 1024);
            ...
            for (String rawHtml : rawHtmlList) {
                String cleanXhtml = sanitizeToXhtml(rawHtml);
    ...
    /**
     * Sanitiza y convierte código HTML suelto en XHTML estricto compatible con Open HTML to PDF.
     */
    public static String sanitizeToXhtml(String rawHtml) {
        if (rawHtml == null || rawHtml.trim().isEmpty()) {
            return "";
        }

        // 1. Corregir URLs de imágenes sin extensión
        String htmlWithFixedImages = fixImageUrls(rawHtml);
        
        Document doc = Jsoup.parse(htmlWithFixedImages);
    ...
    private static boolean processChunk(List<String> htmlList, File outputFile, boolean fitToPage) {
